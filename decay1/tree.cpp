...
void tree::GetEvent()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ipt->GetEntry(jentry);
    if((*br_x).size()==0) continue;
    //front-back correlation
    if(xesum<14000 && abs(xesum-yesum)>200) continue;
    if(xesum>14000 && abs(xesum-yesum)>500) continue; 
    Double_t de=0;
    if((*br_de).size()>0) de=(*br_de)[0].e;
    ...
    y=yeid/yse;
    if((mesum>10 || de>4000) && xesum>10000 && xesum<50000){//recoil
      Recoil r;
      r.e=xesum;
      r.x=x;
      r.y=y;
    ...
    if(mesum<10 && de<1000 && xesum<10000){//decay
      Decay d;
      d.e=xesum;
      d.x=x;
      d.y=y;
...
{
  for(int i=0;i<13;i++){
    if(e>peak[i]-30 && e<peak[i]+30) return i+1;
  }
  return 0;
}
